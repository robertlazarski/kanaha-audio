...
int audio_search_service_invoke_json_impl(
    ...
    if (strcmp(action, "searchKeywords") == 0) {
        ...
        /* Optional decoder priming (R2): passed with this call only, so one
         * request cannot change what another hears. */
        ...
        int rc = whisper_bridge_search_keywords(
            audio_file, keyword_ptrs, num_keywords, initial_prompt, &result
        );
     ...
     * ================================================================ */
        ...
        int rc = whisper_bridge_transcribe(audio_file, initial_prompt, &result);
